...
This project now requires **Go 1.24** for the latest features and performance improvements. To check your Go version:

```bash
go version
```

To update Go to version 1.24, visit [golang.org/dl](https://golang.org/dl/) or use your package manager.
...
You can also modify these constants in `main.go`:
...
docker run -p 8080:8080 -e PORT=8080 tempfile

# Run with volume for persistent uploads directory
docker run -p 3000:3000 -v $(pwd)/uploads:/app/uploads tempfile
```
